...
a=AT24CXX_ReadOneByte(0xa000);
		 ...
		 if(KEY_down==0)
		 {
		  ...
		  AT24CXX_WriteLenByte_sy(0xa000,a,1);   //存储CT_para到eeprom
		 break;
	  ...
	  case 2:				//投入延时参数设置
	  {
		 ...
		 AT24CXX_WriteLenByte_sy(0x1000,DELAY_ON_para,1);  //存储DELAY_ON_para到eeprom
		 break;
	  }
	  case 3:		  //切除延时参数设置
	  {
		 ...
		 if(KEY_down==0)
		 {
			...
			if(DELAY_OFF_para<1)DELAY_OFF_para=60;
			 ...
			 AT24CXX_WriteLenByte_sy(0x2000,DELAY_OFF_para,1);  //存储DELAY_ON_para到eeprom
  ...
  if(KEY_up==0)
		 ...
		 if(KEY_down==0)
		 {
			...
			if(COS_ON_para<80)COS_ON_para=100;
			 ...
			 AT24CXX_WriteLenByte_sy(0x3000,COS_ON_para,1);  //存储DELAY_ON_para到eeprom
		 break;
	  ...
	  case 5:		//切除COS参数设置
	  {
		 ...
		 if(KEY_down==0)
		 {
            ...
            if(COS_OFF_para<80)COS_OFF_para=100;
					 ...
					 AT24CXX_WriteLenByte_sy(0x4000,COS_OFF_para,1);  //存储DELAY_ON_para到eeprom
	  ...
	  case 7:	   //投入门限参数设置
	  {
		...
		ON_HOLD_para=AT24CXX_ReadOneByte(0x5000); 
		 ...
		 if(KEY_down==0)
		 {
		  ...
		  //存储CT_para到eeprom
		 AT24CXX_WriteLenByte_sy(0x5000,ON_HOLD_para,1);  //存储DELAY_ON_para到eeprom
	  ...
	  case 8:	  //切除门限参数设置
	  {
		...
		OFF_HOLD_para=AT24CXX_ReadOneByte(0x6000); 
		 ...
		 if(KEY_down==0)
		 {
		  ...
		  //存储CT_para到eeprom
		 AT24CXX_WriteLenByte_sy(0x6000,OFF_HOLD_para,1);  //存储DELAY_ON_para到eeprom
	...
	//	WriteAll_1621(22,numHU_prot+2,2);
		 ...
		 if(KEY_down==0)
		 {
			...
			if(HU_PROT_para<1)HU_PROT_para=100;
		 		 ...
		 		 AT24CXX_WriteLenByte_sy(0x7000,HU_PROT_para,1);  //存储DELAY_ON_para到eeprom
	...
	//   WriteAll_1621(22,numHI_prot+2,2);
		 ...
		 if(KEY_down==0)
		 {
			...
			if(HI_PROT_para<1)HI_PROT_para=100;
		 		 		 ...
		 		 		 AT24CXX_WriteLenByte_sy(0x8000,HI_PROT_para,1);  //存储DELAY_ON_para到eeprom
